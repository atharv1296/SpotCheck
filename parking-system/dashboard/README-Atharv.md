...
- Install Python driver: `pip install oracledb`
- Optionally install Oracle Instant Client (for thick mode) using `install_oracle_client.ps1`
- Configure environment variables (you can copy `.env.sample` to `.env`):
   - ORACLE_DSN or ORACLE_HOST/ORACLE_PORT/ORACLE_SERVICE_NAME
   - ORACLE_USER and ORACLE_PASSWORD
   - Optional: ORACLE_THICK_MODE=1 and ORACLE_CLIENT_LIB_DIR
