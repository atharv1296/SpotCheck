"""
Django Management Command: Setup SQLite Database
Forbes Marshall SpotCheck - SQLite Database Setup

This command sets up the SQLite database with all required tables and initial data.
Use this when Oracle Client is not available.
"""
...
class Command(BaseCommand):
    help = 'Setup SQLite database with all required tables and initial data'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("📊 SQLITE DATABASE SETUP - FORBES MARSHALL SPOTCHECK"))
        self.stdout.write("=" * 70)
        ...
        try:
            ...
            self.stdout.write("\n" + "=" * 70)
            self.stdout.write(self.style.SUCCESS("✅ SQLITE DATABASE SETUP COMPLETED SUCCESSFULLY!"))
            self.stdout.write("🌐 Access the system at: http://127.0.0.1:8000/gate/")
            self.stdout.write("\n💡 To switch to Oracle later, install Oracle Client and update settings.py")
            self.stdout.write("=" * 70)
    ...
    def test_connection(self):
        ...
        self.stdout.write("🔌 Testing SQLite database connection...")
        
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT 'SQLite Connected' as status")
    ...
    def create_sample_vehicles(self):
        ...
        sample_vehicles = [
            {'license_plate': 'MH12AB1234', 'vehicle_type': 'Hatchback', 'owner_name': 'John Doe'},
    ...
    def show_database_status(self):
        ...
        try:
            ...
            slot_categories = ['Hatchback', 'Sedan', 'SUV', 'Large Vehicle']
            for category in slot_categories:
                count = ParkingSlot.objects.filter(slot_type=category).count()
                self.stdout.write(f"   🅿️  {category} slots: {count}")
